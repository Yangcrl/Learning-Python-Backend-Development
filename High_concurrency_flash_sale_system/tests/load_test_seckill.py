...
try:
    ...
    print(f"\n当前剩余库存: {remaining_stock}")
    
    # 计算理论剩余库存
    initial_stock = 100
    theoretical_remaining = initial_stock - successful_requests
    print(f"理论剩余库存: {theoretical_remaining}")
    
    if int(remaining_stock) >= theoretical_remaining:
        print("✓ 未发现超卖现象")
    else:
        print("✗ 发现超卖现象！")
finally:
